...
export default function SmsTemplates() {
  ...
  const handleOpenDialog = (template?: SmsTemplate) => {
    if (template) {
      ...
      setFormData({ name: "", template_key: "", content: "" });
    }
    setDialogOpen(true);
  };

  const handleSave = async () => {
    if (!formData.name || !formData.template_key) {
      toast.error("Navn og nøgle er påkrævet");
      return;
    }
      ...
      <main className="flex-1 overflow-auto">
        <div className="container max-w-6xl py-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-3xl font-bold">SMS-skabeloner</h1>
              <p className="text-muted-foreground mt-2">
                Administrer SMS-skabeloner. Brug {"{{fornavn}}"} og {"{{rolle}}"} som variable.
              </p>
            </div>
