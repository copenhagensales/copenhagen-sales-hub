...
export class TwilioVoiceManager {
  ...
  private onDebugUpdate?: (info: { tokenLength?: number; deviceError?: string; [key: string]: any }) => void;

  constructor(
    identity: string,
    onCallStatusChange?: (status: string, call?: Call) => void,
    onDebugUpdate?: (info: { tokenLength?: number; deviceError?: string; [key: string]: any }) => void,
  ) {
    ...
    try {
      ...
      this.device.on("error", (error) => {
        ...
        if (error?.code === 20101) {
          ...
          errorDetails =
            "AccessTokenInvalid: JWT signature or structure is invalid. Check that credentials are from same Twilio account.";
